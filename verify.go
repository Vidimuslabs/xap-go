...
func (v *Verifier) Verify(in VerifyInput) VerificationResult {
	...
	if in.CommitmentEnvelope != nil {
		...
		if err != nil {
			...
			// Before comparing the commitment's agent identity to anything, a
			// PRESENT identity has to be well formed on its own: its kind must be
			// a recognized discriminant and must agree with the material present
			// (SCHEMA.md field 122). This is a property of the commitment alone,
			// so it holds or fails with no MAT, and it fails rather than reports
			// not-performed, because a kind its material does not support is a
			// defect in what was presented.
			//
			// Guarded on identityUnset for the same reason the MAT identity is
			// (mat.go ValidateStructure): an unset identity is the protocol's
			// "open identity", not a malformed one. The gap this closes is a kind
			// that lies about material it DOES carry — kind "attestation" over a
			// bare key — not an identity that carries nothing.
			if !identityUnset(sc.Commitment.AgentIdentity) {
				if err := sc.Commitment.AgentIdentity.Validate(); err != nil {
					add("agent_identity_wellformed", false, err.Error())
				} else {
					add("agent_identity_wellformed", true,
						fmt.Sprintf("commitment agent identity kind %q is well formed", sc.Commitment.AgentIdentity.Kind))
				}
			} else {
				notPerformed("agent_identity_wellformed",
					"the commitment declares no agent identity to check")
			}
